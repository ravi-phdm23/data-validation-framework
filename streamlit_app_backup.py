#!/usr/bin/env python3
"""
Streamlit Frontend for BigQuery Test Scenarios - Modular Version
...
def show_excel_mapping_tab():
    ...
    with col1:
        ...
        if uploaded_file is not None:
            ...
            if error:
                ...
                # Show scenarios preview and SQL section if scenarios exist in session state
                if 'excel_scenarios' in st.session_state and st.session_state['excel_scenarios']:
                    ...
                    with st.expander("🔍 SQL Preview", expanded=False):
                        ...
                        # Use a simple selectbox with string values (more stable)
                        selected_scenario_name = st.selectbox(
                            "Choose scenario to view SQL:",
                            scenario_options,
                            key="scenario_sql_selector"
                        )
    ...
    if 'excel_scenarios' in st.session_state and st.session_state['excel_scenarios']:
        ...
        with col1:
            st.metric("Generated Scenarios", len(st.session_state['excel_scenarios']))
